...
#' function connectivity best predicts a response in a generalized linear model
#' (GLM). This is a scale of effect optimization: `alpha` is chosen to maximize the
#' fit of `glm(formula, family)`, where one predictor is the connectivity produced by
#' [ifc()].
#'
#' @param formula A model formula for the GLM. The predictor named `connectivity` is
#'   reserved: it is recomputed by [ifc()] at each candidate `alpha` and added to the
#'   data, so it does not need to exist in `data`. Any other terms must be columns of
#'   `data`. For example `occupied ~ connectivity + habitat`.
#' @param data A data frame with one row per site, in the same order as the rows of
#'   `dist_mat`. It holds the response and any covariates in `formula` (but not
#'   `connectivity`, which is generated internally).
...
#'   example `binomial`, `poisson`, `gaussian`). (Default = `binomial()`)
...
ifc_optim <- function(formula,
  ...
  if(nrow(data) != nrow(dist_mat)){
    stop("`data` must have the same number of rows as `dist_mat` (one per site).")
  }
  ...
  ## ---- fit one (model, area_transform) combination ----
  ## Closure over the shared arguments; returns a single `ifc_optim` object.
  fit_one <- function(this_model, this_transform) {

    ll_data <- function(alpha, dat) {
      dat$connectivity <- as.numeric(ifc(alpha, dist_mat, model = this_model,
                                         patch_area = patch_area, occ_sites = occ_sites,
                                         area_transform = this_transform))
      fit <- tryCatch(suppressWarnings(stats::glm(formula, data = dat, family = family)),
                      error = function(e) NULL)
      if(is.null(fit)) return(NA_real_)
      as.numeric(stats::logLik(fit))
    }

    ## Optimize alpha: grid scan, then refine around the best cell
    optim_alpha <- function(dat) {
      grid <- exp(seq(log(alpha_range[1]), log(alpha_range[2]), length.out = n_grid))
      gll  <- vapply(grid, function(a) ll_data(a, dat), numeric(1))
      if(all(is.na(gll))){
        stop("The GLM could not be fit at any alpha. Check `formula`, `family`, and `data`.")
      }
      k   <- which.max(gll)
      opt <- stats::optimize(function(a) -ll_data(a, dat),
                             lower = grid[max(1, k - 1)], upper = grid[min(n_grid, k + 1)])
      list(alpha = opt$minimum, grid = grid, gll = gll, logLik = -opt$objective)
    }

    res <- optim_alpha(data)
    best_data <- data
    best_data$connectivity <- as.numeric(ifc(res$alpha, dist_mat, model = this_model,
                                             patch_area = patch_area, occ_sites = occ_sites,
                                             area_transform = this_transform))
    best <- stats::glm(formula, data = best_data, family = family)
    ...
    alpha_ci_boot <- NULL
    if(n_boot > 0){
      sims <- stats::simulate(best, nsim = n_boot)
      boot <- vapply(seq_len(n_boot), function(b) {
        dat_b <- data
        dat_b[[resp_name]] <- sims[[b]]
        tryCatch(optim_alpha(dat_b)$alpha, error = function(e) NA_real_)
      }, numeric(1))
      a_lo <- (1 - conf_level) / 2
    ...
    structure(list(alpha = res$alpha,
                   ...
                   boot = boot,
                   logLik = res$logLik,
                   AIC = stats::AIC(best) + 2,         # count the alpha parameter
                   model = best,
                   profile = data.frame(alpha = res$grid, logLik = res$gll),
                   family = best$family$family,
                   ifc = list(model = this_model, area_transform = this_transform),
  ...
  ## ---- single combination: return one ifc_optim object (unchanged behavior) ----
  if(nrow(combos) == 1L){
    return(fit_one(combos$model[1], combos$area_transform[1]))
  }

  ## ---- multiple combinations: fit each and return an ifc_optim_set ----
  fits <- Map(fit_one, combos$model, combos$area_transform)
